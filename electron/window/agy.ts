import { BrowserWindow, screen } from "electron";
import path from "node:path";

const GLOW_HTML = /* html */ `<!DOCTYPE html>
<html>
<head>
<style>
  @property --a1 {
    syntax: "<angle>";
    initial-value: 0deg;
    inherits: false;
  }
  @property --a2 {
    syntax: "<angle>";
    initial-value: 120deg;
    inherits: false;
  }

  * { margin: 0; padding: 0; }

  body {
    background: transparent;
    overflow: hidden;
    width: 100vw;
    height: 100vh;
  }

  .glow {
    position: fixed;
    inset: 0;
    opacity: 0;
    transition: opacity 1s cubic-bezier(0.4, 0, 0.2, 1);
    --radius: 14px;
  }

  .glow.active { opacity: 1; }

  .wave {
    position: absolute;
    inset: 0;
    border-radius: var(--radius);
  }

  .fade-mask {
    mask:
      linear-gradient(to right,  #000, transparent var(--fade)) left  / 50% 100% no-repeat,
      linear-gradient(to left,   #000, transparent var(--fade)) right / 50% 100% no-repeat,
      linear-gradient(to bottom, #000, transparent var(--fade)) top   / 100% 50% no-repeat,
      linear-gradient(to top,    #000, transparent var(--fade)) bottom/ 100% 50% no-repeat;
  }

  .border-mask {
    mask:
      linear-gradient(#000, #000) content-box exclude,
      linear-gradient(#000, #000);
  }

  /* Layer 1 — visible border with inward fade */
  .wave-1 {
    --fade: 60px;
    background: conic-gradient(
      from var(--a1),
      #7c3aed, #6366f1, #3b82f6, #06b6d4,
      #10b981, #8b5cf6, #c084fc, #7c3aed
    );
    animation: spin-1 4s linear infinite, breathe-1 3s ease-in-out infinite;
  }

  /* Layer 2 — wide ambient glow, fades deep into screen */
  .wave-2 {
    --fade: 160px;
    background: conic-gradient(
      from var(--a2),
      #06b6d4, #34d399, #10b981, #3b82f6,
      #8b5cf6, #a78bfa, #06b6d4
    );
    filter: blur(16px);
    opacity: 0.6;
    animation: spin-2 7s linear infinite, breathe-2 5s ease-in-out infinite;
  }

  /* Traveling highlight — bright arc sweeping the border */
  .highlight {
    padding: 8px;
    background: conic-gradient(
      from var(--a1),
      transparent 0%,
      transparent 30%,
      rgba(255, 255, 255, 0.95) 45%,
      #a5b4fc 50%,
      rgba(255, 255, 255, 0.95) 55%,
      transparent 70%,
      transparent 100%
    );
    filter: blur(5px);
    opacity: 0.7;
    animation: spin-1 3s linear infinite, highlight-pulse 3s ease-in-out infinite;
  }

  @keyframes spin-1 { to { --a1: 360deg; } }
  @keyframes spin-2 { to { --a2: -240deg; } }

  @keyframes breathe-1 {
    0%, 100% { opacity: 1; }
    50%      { opacity: 0.7; }
  }

  @keyframes breathe-2 {
    0%, 100% { opacity: 0.6; }
    50%      { opacity: 0.4; }
  }

  @keyframes highlight-pulse {
    0%, 100% { opacity: 0.7; }
    50%      { opacity: 0.3; }
  }

  /* ─── Overlay panel ─── */
  .overlay {
    position: fixed;
    bottom: 10%;
    left: 50%;
    transform: translateX(-50%) translateY(10px);
    max-width: 520px;
    min-width: 80px;
    padding: 10px 22px;
    background: rgba(12, 12, 12, 0.82);
    backdrop-filter: blur(24px);
    -webkit-backdrop-filter: blur(24px);
    border-radius: 16px;
    border: 1px solid rgba(255, 255, 255, 0.06);
    box-shadow: 0 8px 32px rgba(0, 0, 0, 0.5);
    color: rgba(255, 255, 255, 0.9);
    font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", system-ui, sans-serif;
    font-size: 14px;
    line-height: 1.5;
    text-align: center;
    opacity: 0;
    transition: opacity 500ms ease, transform 500ms ease;
    pointer-events: none;
    z-index: 10;
  }

  .overlay.active {
    opacity: 1;
    transform: translateX(-50%) translateY(0);
  }

  .content {
    word-break: break-word;
    white-space: pre-wrap;
  }

  .content:empty { display: none; }

  .overlay[data-phase="recording"] .content:not(:empty)::after,
  .overlay[data-phase="responding"] .content:not(:empty)::after {
    content: "";
    display: inline-block;
    width: 2px;
    height: 1em;
    background: rgba(255, 255, 255, 0.7);
    margin-left: 3px;
    vertical-align: text-bottom;
    animation: cursor-blink 1s step-end infinite;
  }

  @keyframes cursor-blink { 50% { opacity: 0; } }

  .thinking {
    display: none;
    gap: 8px;
    justify-content: center;
    align-items: center;
    padding: 4px 0;
  }

  .overlay[data-phase="thinking"] .thinking { display: flex; }
  .overlay[data-phase="thinking"] .content { display: none; }

  .dot {
    width: 6px;
    height: 6px;
    border-radius: 50%;
    background: rgba(255, 255, 255, 0.8);
    animation: dot-bounce 1.4s ease-in-out infinite;
  }

  .dot:nth-child(2) { animation-delay: 0.16s; }
  .dot:nth-child(3) { animation-delay: 0.32s; }

  @keyframes dot-bounce {
    0%, 80%, 100% { transform: scale(0.6); opacity: 0.3; }
    40%           { transform: scale(1);    opacity: 1; }
  }
</style>
</head>
<body>
  <div class="glow" id="glow">
    <div class="wave fade-mask wave-2"></div>
    <div class="wave fade-mask wave-1"></div>
    <div class="wave border-mask highlight"></div>
  </div>

  <div class="overlay" id="overlay" data-phase="idle">
    <p class="content" id="content"></p>
    <div class="thinking">
      <span class="dot"></span>
      <span class="dot"></span>
      <span class="dot"></span>
    </div>
  </div>

  <script>
    const glowEl = document.getElementById('glow');
    const overlay = document.getElementById('overlay');
    const content = document.getElementById('content');

    let phase = 'idle';
    let fadeTimer = null;
    let fallbackTimer = null;

    function setPhase(p) {
      phase = p;
      overlay.dataset.phase = p;
      if (fadeTimer) { clearTimeout(fadeTimer); fadeTimer = null; }

      if (p === 'idle') {
        fadeTimer = setTimeout(() => {
          overlay.classList.remove('active');
          glowEl.classList.remove('active');
        }, 3000);
      } else {
        overlay.classList.add('active');
      }
    }

    function formatTool(name) {
      return name.replace(/_/g, ' ').replace(/\\b\\w/g, function(c) { return c.toUpperCase(); });
    }

    /* Recording glow — keep border active until idle */
    window.electron.onRecordingGlow((active) => {
      if (fallbackTimer) { clearTimeout(fallbackTimer); fallbackTimer = null; }
      if (active) {
        glowEl.classList.add('active');
        content.textContent = '';
        setPhase('recording');
      } else {
        fallbackTimer = setTimeout(() => {
          if (phase === 'recording') setPhase('idle');
        }, 5000);
      }
    });

    /* Streaming transcription */
    window.electron.onTranscriptionDelta((text) => {
      if (phase === 'recording') content.textContent += text;
    });

    /* Phase transitions from main process */
    window.electron.onGlowPhase((p) => {
      if (fallbackTimer) { clearTimeout(fallbackTimer); fallbackTimer = null; }
      if (p === 'thinking') content.textContent = '';
      setPhase(p);
    });

    /* Tool execution */
    window.electron.onToolExecuting((data) => {
      content.textContent = formatTool(data.name) + '...';
      setPhase('executing');
    });

    /* LLM response chunks */
    window.electron.onChatChunk((chunk) => {
      if (phase === 'thinking' || phase === 'executing') {
        content.textContent = '';
        setPhase('responding');
      }
      content.textContent += chunk;
    });
  </script>
</body>
</html>`;

export function createAgyWindow(dirname: string): BrowserWindow {
  const { x, y, width, height } = screen.getPrimaryDisplay().bounds;

  const win = new BrowserWindow({
    x,
    y,
    width,
    height,
    transparent: true,
    frame: false,
    hasShadow: false,
    skipTaskbar: true,
    resizable: false,
    focusable: false,
    show: false,
    webPreferences: {
      preload: path.join(dirname, "preload.mjs"),
    },
  });

  win.setAlwaysOnTop(true, "screen-saver");
  win.setIgnoreMouseEvents(true);
  win.setVisibleOnAllWorkspaces(true);

  win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(GLOW_HTML)}`);

  win.once("ready-to-show", () => win.showInactive());

  return win;
}
